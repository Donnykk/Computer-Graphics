...
namespace NRenderer
{
    AssetView::AssetView(const Vec2 &position, const Vec2 &size, UIContext &uiContext, Manager &manager)
        : View(position, size, uiContext, manager), tempProp("property", Property::Wrapper::IntType{})
    {
        currMtlIndex = -1;
        currLightIndex = -1;
        isPropKeyValid = true;
...
#pragma endregion TEMP_PROP_EDITOR
    ...
    void AssetView::modelTab()
    {
        ...
        {
            const auto flags = ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_Resizable | ImGuiTableFlags_RowBg;
        ...
        ImGui::BeginChild("ModelSetting");
        {
            ...
            if (uiContext.previewMode == UIContext::PreviewMode::PREVIEW_MODEL && uiContext.previewModel >= 0 && uiContext.previewModel < mis.size())
            {
            ...
            else if (uiContext.previewMode == UIContext::PreviewMode::PREVIEW_NODE && uiContext.previewNode >= 0 && uiContext.previewNode < nis.size())
            {
...
#undef SET
    ...
    void AssetView::materialTab()
    {
        ...
        const auto flags = ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg;
        ...
        if (ImGui::BeginTable("LightItems##Table", 3, flags))
        {
            ...
            for (auto &l : lights)
            {
                ...
                if (l.light->type == Light::Type::AREA)
                    ...
                    typeStr = "Point Light";
                else if (l.light->type == Light::Type::SPOT)
                    typeStr = "Spot";
                ImGui::TextUnformatted(typeStr.c_str());
                i++;
            }
